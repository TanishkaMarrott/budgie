...
_IAC_MARKERS = ("terraform apply", "terraform destroy", "pulumi up",
                "cloudformation deploy", "cloudformation create-stack", "cdk deploy")
...
def check(command: str, cap_hourly: float = 2.0) -> Decision:
    ...
    if intents:
        ...
        return aggregate([_estimate(i) for i in intents], cap_hourly, in_loop)
